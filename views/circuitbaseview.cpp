...
QColor CircuitBaseView::gXYLineColor = QColor(0,255,255);

CircuitBaseView::CircuitBaseView(QWidget *parent)
...
CircuitBaseView::CircuitBaseView(QGraphicsScene *scene, QWidget *parent)
    : QGraphicsView(parent)
    , IS_MOVE(false)
    , IS_MOVED(false)
    , mDefaultBkColor(QColor(218,218,218))
...
void CircuitBaseView::init()
{
    ...
    connect(pChangeBkColor, &QAction::triggered, this, &CircuitBaseView::slotChangeBkColor);
...
void CircuitBaseView::enbaleDrawLine(bool flag)
{
    bDrawLine = flag;
    IS_MOVED  = true;
...
//    void *ptr = item;
//    qDebug() << (int *)&ptr;
//    mLineObject = (CircuitLineObject *)ptr;
//    mLineObject->setPos(gEndPoint);
    QPen pen(Qt::CustomDashLine);
    QVector<qreal> dashes;
    dashes << 2 << 4;
    pen.setDashPattern(dashes);
    pen.setColor(CircuitBaseView::gXYLineColor);
    pen.setWidth(0);
    pXLine->setPen(pen);
    pYLine->setPen(pen);
...
void CircuitBaseView::mouseMoveEvent(QMouseEvent *event)
{
    if(IS_MOVE)
    {
        IS_MOVED = true;
...
void CircuitBaseView::mousePressEvent(QMouseEvent *event)
{
...
void CircuitBaseView::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    if(event->button() == Qt::RightButton)
    {
        ...
        if(bDrawLine && (!bIsDrawingLine))
        {
            ...
            CircuitLineObject *item = new CircuitLineObject();
...
void CircuitBaseView::wheelEvent(QWheelEvent *event)
{
    event->delta() > 0 ? scale(1.05,1.05) : scale(0.95,0.95);
}

void CircuitBaseView::enterEvent(QEvent *event)
{
    Q_UNUSED(event);
    if(!bDrawLine) return;
...
void CircuitBaseView::leaveEvent(QEvent *event)
{
    Q_UNUSED(event);
    if(!bDrawLine) return;
    pXLine->setVisible(false);
...
void CircuitBaseView::slotChangeBkColor()
{
